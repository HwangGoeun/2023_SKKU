...
min_x = 5      # ROI 가로 최소     // 좌표계 4사분면
min_y = 450     # ROI 세로 최소
max_x = width - 150   # ROI 가로 최대
max_y = height - 50    # ROI 세로 최소
...
while True : 
    ...
    limited_polylines_list = [[min_x, max_y],  [max_x,max_y], [max_x-200, min_y], [min_x+200, min_y]]   # bird eye view ROI세팅  꼭짓점 4개
    limited_polylines_list_1 = [[min_x-2, max_y+2],  [max_x+2, max_y+2], [max_x+2-200, min_y-2], [min_x-2+200, min_y-2]]   #offset이라서 +-2는 수정하지 말것
    ...
    leftLine = cv2.line(dst_binaryzation, (width/4, 0), (width/4, height), (0, 255, 0), 2, cv2.LINE_AA) #가로로 4등분 
    ...
    cv2.putText(frame, direction, (width/2,height/4), cv2.FONT_ITALIC, 1, (0,0,255), 2)  # 보기쉽게 흑백화면에서 4등분표시
